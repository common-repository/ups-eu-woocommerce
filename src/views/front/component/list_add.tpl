{if isset($dataObject->data->info_rateShopADD)}
    {if $dataObject->data->info_rateShopADD->check_api_response eq "ok"}
                                    ...
                                    <tr>
                                        <td style="width: 33px;">
                                            <input {if $item_service->Service->ID_Local eq $dataObject->data->min_total_price_service->id_service}checked="checked"{/if}
                                                   onclick="__upsMapShipping.func.checkout_update('{$item_service->Service->ID_Local}', 'ADD', '{$itemRateTimeInTransit->custom->monetary_value}');"
