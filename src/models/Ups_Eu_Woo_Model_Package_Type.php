...
new \UpsEuWoocommerce\Ups_Eu_Woo_Main_Autoloader('Ups_Eu_Woo_Model_Package_Type');

class Ups_Eu_Woo_Model_Package_Type
{
    private $show_params_call_api = false;
    private $fee_shipping = '_fee_shipping';
    public $descriptions_package_default = [
        'kgs' => "kilograms",
        'lbs' => "Pounds",
        'cm' => "centimeter",
        'inch' => "inch"
    ...
    private $list_country_cash_on_dilevery = ["BE", "NL", "FR", "ES", "PL", "IT", "DE", "UK", "AT", "BG", "HR", "CY", "CZ", "DK", "EE", "FI", "GR", "HU", "IE", "LV", "LT", "LU", "MT", "PT",  "RO", "SK", "SI", "SE", "NO", "RS", "CH", "IS", "JE", "TR"];
    ...
    //                 $package_api_param->dimension->length = $package_item->length = sprintf('%0.2f', $ups_pack['length']);
    //                 $package_api_param->dimension->width = $package_item->width = sprintf('%0.2f', $ups_pack['width']);
    //                 $package_api_param->dimension->height = $package_item->height = sprintf('%0.2f', $ups_pack['height']);
    ...
    //             $package_api_param->package_weight->weight = $package_item->weight = sprintf('%0.2f', $ups_pack['weight']);
    ...
    function ups_eu_woo_pack_order_items($order_id)
    {
        $model_package = new Ups_Eu_Woo_Model_Package_Default();
        $model_config  = new Ups_Eu_Woo_Model_Config();
        $package_default_item = false;
        ...
        if ($package_setting_option == 1) {
            $list_package = $model_package->get_list_data_by_condition();
            $package_default_item = $this->get_package_valid($list_package,$order_id);
            ...
            if ($package_default_item) {
                ...
                if (0 != $package_include_dimension) {
                    $package_api_param->dimension = new \stdClass();
                    $package_api_param->dimension->code = strtoupper($package_default_item->unit_dimension);
                    $package_api_param->dimension->description = $dimension_description;
                    $package_item->length = "{$package_default_item->length}";
                    $package_item->width = "{$package_default_item->width}";
                    $package_item->height = "{$package_default_item->height}";
                    $package_item->unit_dimension = $package_default_item->unit_dimension;
                }

                $package_api_param->package_weight = new \stdClass();
                $package_api_param->package_weight->code = strtoupper($package_default_item->unit_weight);
                $package_api_param->package_weight->description = $weight_description;
                $package_item->weight = "{$package_default_item->weight}";
                $package_item->unit_weight = $package_default_item->unit_weight;
            ...
            $ups_packages = $this ->ups_eu_woo_pack_cart_items($order_id);
            //  echo "<pre>";print_r($ups_packages);die();
            $list_package_api_param = [];
            $order_package_type = [];
            if (is_array($ups_packages) || is_object($ups_packages)){
                foreach ($ups_packages as $key => $ups_pack) {
                    $package_api_param = new \stdClass();
                    $package_item = new \stdClass();
                    if (isset($ups_pack['unit_dimension']) && isset($ups_pack['length']) && isset($ups_pack['width']) && isset($ups_pack['height'])) {
                        ...
                        $package_api_param->dimension = new \stdClass();
                        $package_api_param->dimension->code = strtoupper($ups_pack['unit_dimension']);
                        $package_api_param->dimension->description = $model_pack->descriptions_package_default[$ups_pack['unit_dimension']];
                        $package_api_param->dimension->length = $package_item->length = substr(sprintf('%0.2f', $ups_pack['length']), 0, 6);
                        $package_api_param->dimension->width = $package_item->width = substr(sprintf('%0.2f', $ups_pack['width']), 0, 6);
                        $package_api_param->dimension->height = $package_item->height = substr(sprintf('%0.2f', $ups_pack['height']), 0, 6);
                        $package_item->unit_dimension = $ups_pack['unit_dimension'];
                    }
                    // Set weight info
                    $package_api_param->package_weight = new \stdClass();
                    $package_api_param->package_weight->code = strtoupper($ups_pack['unit_weight']);
                    $package_api_param->package_weight->description = $model_pack->descriptions_package_default[$ups_pack['unit_weight']];
                    $package_api_param->package_weight->weight = $package_item->weight = substr(sprintf('%0.2f', $ups_pack['weight']), 0, 6);
                    $package_item->unit_weight = $ups_pack['unit_weight'];
    ...
    function ups_eu_woo_pack_cart_items($order_id)
    {
        // Get config package include dimension
        $model_config  = new Ups_Eu_Woo_Model_Config();
        $selected_pac_algo = 3;
        ...
        if ($selected_pac_algo == 1) {
            return $this->ups_eu_woo_pack_cart_items_individually($order_id);
        }
        if ($selected_pac_algo == 2) {
            return $this->ups_eu_woo_pack_cart_items_with_weight($order_id);
        }
        if ($selected_pac_algo == 3) {
            return $this->ups_eu_woo_pack_cart_items_with_box($order_id);
        }
    }

    private function ups_eu_woo_pack_cart_items_individually($order_id)
    {
        // Get config package include dimension
        $model_config  = new Ups_Eu_Woo_Model_Config();
        ...
        $order_data = $order->get_data($order_id);
        if (empty($order)) {
            return;
        }
        $final_out = [];
    ...
    private function ups_eu_woo_pack_cart_items_with_weight($order_id)
    {
        // Get config package include dimension
        $model_config  = new Ups_Eu_Woo_Model_Config();
        $pac_max_weight = 1;
        ...
        foreach ($order->get_items() as $item_key => $item ){
            ...
            if (!empty($product)) {
                ...
                $prod_data['product_quantity'] = $item['quantity'];
                $weight_pack->add_item($prod_data['product_weight'], $prod_data, $item['quantity']);
            }
        }
    ...
    private function ups_eu_woo_pack_cart_items_with_box($order_id)
    {
        $model_product_dimension = new Ups_Eu_Woo_Model_Product_Dimension();
        $all_saved_boxes = $model_product_dimension->ups_eu_woo_get_all();
    ...
    {
        if ($object_call->RateTimeInTransit && $object_call->RateTimeInTransit[$service_id]) {
    ...
    public function get_package_valid($list_package,$order_id)
    {
        // number of items
        $order = wc_get_order( $order_id ); 
